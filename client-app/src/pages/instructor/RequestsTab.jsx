import { useState, useEffect } from 'react';
import {
  Box, Typography, Alert, CircularProgress, Chip, Button,
  Table, TableHead, TableBody, TableRow, TableCell, TableContainer, Paper,
  Dialog, DialogTitle, DialogContent, DialogActions,
} from '@mui/material';
import { getOrders, approveOrder, rejectOrder, deleteOrder } from '../../api/orders';
import { getCourses } from '../../api/courses';

const statusMap = {
  pending: { label: 'Ожидает', color: 'warning' },
  approved: { label: 'Одобрена', color: 'success' },
  rejected: { label: 'Отклонена', color: 'error' },
};

export default function RequestsTab() {
  const [orders, setOrders] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [confirmAction, setConfirmAction] = useState(null);

  const load = async () => {
    setLoading(true);
    setError('');
    try {
      const [allOrders, courses] = await Promise.all([getOrders(), getCourses()]);
      const courseIds = new Set(courses.map((c) => c.id));
      const filtered = allOrders.filter((o) =>
        (o.orderItems || []).some((oi) => courseIds.has(oi.courseId)),
      );
      setOrders(filtered);
    } catch {
      setError('Не удалось загрузить заявки');
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => { load(); }, []);

  const handleAction = async () => {
    if (!confirmAction) return;
    try {
      setError('');
      const { type, id } = confirmAction;
      if (type === 'approve') await approveOrder(id);
      else if (type === 'reject') await rejectOrder(id);
      else if (type === 'delete') await deleteOrder(id);
      setConfirmAction(null);
      await load();
    } catch (e) {
      setError('Ошибка: ' + (e.message || e));
    }
  };

  const confirmLabel = () => {
    if (!confirmAction) return '';
    if (confirmAction.type === 'approve') return 'Одобрить заявку?';
    if (confirmAction.type === 'reject') return 'Отклонить заявку?';
    return 'Удалить заявку?';
  };

  if (loading) {
    return (
      <Box display="flex" justifyContent="center" py={6}>
        <CircularProgress />
      </Box>
    );
  }

  return (
    <Box>
      <Typography variant="h5" gutterBottom>Заявки на доступ</Typography>

      {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}

      <TableContainer component={Paper}>
        <Table>
          <TableHead>
            <TableRow>
              <TableCell>ID</TableCell>
              <TableCell>Пользователь</TableCell>
              <TableCell>Дата</TableCell>
              <TableCell>Статус</TableCell>
              <TableCell>Курсы</TableCell>
              <TableCell>Действия</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {orders.map((o) => {
              const st = statusMap[o.status] || { label: o.status, color: 'default' };
              const courseNames = (o.orderItems || [])
                .map((oi) => oi.course?.title || `Курс ${oi.courseId}`)
                .join(', ');
              return (
                <TableRow key={o.id}>
                  <TableCell>{o.id}</TableCell>
                  <TableCell>{o.user?.name || '—'}</TableCell>
                  <TableCell>{new Date(o.orderDate).toLocaleDateString('ru-RU')}</TableCell>
                  <TableCell>
                    <Chip label={st.label} color={st.color} size="small" />
                  </TableCell>
                  <TableCell>{courseNames || '—'}</TableCell>
                  <TableCell>
                    {o.status === 'pending' ? (
                      <>
                        <Button size="small" color="success" onClick={() => setConfirmAction({ type: 'approve', id: o.id })}>
                          Одобрить
                        </Button>
                        <Button size="small" color="error" onClick={() => setConfirmAction({ type: 'reject', id: o.id })} sx={{ ml: 1 }}>
                          Отклонить
                        </Button>
                      </>
                    ) : (
                      <Button size="small" color="error" onClick={() => setConfirmAction({ type: 'delete', id: o.id })}>
                        Удалить
                      </Button>
                    )}
                  </TableCell>
                </TableRow>
              );
            })}
            {orders.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} align="center">Заявок нет</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
      </TableContainer>

      <Dialog open={confirmAction !== null} onClose={() => setConfirmAction(null)}>
        <DialogTitle>Подтверждение</DialogTitle>
        <DialogContent>
          <Typography>{confirmLabel()}</Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setConfirmAction(null)}>Отмена</Button>
          <Button
            variant="contained"
            color={confirmAction?.type === 'approve' ? 'success' : 'error'}
            onClick={handleAction}
          >
            Да
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
}
