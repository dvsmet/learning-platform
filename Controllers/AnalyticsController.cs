...
public class AnalyticsController : ControllerBase
{
    ...
    private enum SuccessEnrollmentCategory
    {
        NotStarted,
        Passed,
        FailedOrInProgress
    ...
    private static SuccessEnrollmentCategory ClassifyEnrollmentForSuccessPie(
        AnalyticsScope scope, int courseId, int userId, int passingScorePercent)
    {
        var course = scope.Courses.FirstOrDefault(c => c.Id == courseId);
        if (course == null) return SuccessEnrollmentCategory.FailedOrInProgress;

        var courseQuizzes = scope.Quizzes
            .Where(q => scope.LessonToCourse.GetValueOrDefault(q.LessonId) == courseId)
            .ToList();
        var progresses = scope.ProgressesByCourse.GetValueOrDefault(courseId) ?? [];
        var userProgress = progresses.Where(p => p.UserId == userId).ToList();

        var hasLessonActivity = userProgress.Any(p =>
            IsStatus(p.Status, "started") || IsStatus(p.Status, "completed"));
        var hasQuizActivity = false;
        foreach (var q in courseQuizzes)
        {
            if (scope.RawResults.Any(r => r.QuizId == q.Id && r.UserId == userId))
            {
                hasQuizActivity = true;
                break;
            }
        }

        if (!hasLessonActivity && !hasQuizActivity)
            return SuccessEnrollmentCategory.NotStarted;

        if (courseQuizzes.Count > 0)
        {
            foreach (var q in courseQuizzes)
            {
                var attempts = scope.RawResults.Where(r => r.QuizId == q.Id && r.UserId == userId).ToList();
                if (attempts.Count == 0) return SuccessEnrollmentCategory.FailedOrInProgress;
                var best = attempts.Max(a => q.MaxScore > 0 ? a.Score * 100.0 / q.MaxScore : 0);
                if (best < passingScorePercent) return SuccessEnrollmentCategory.FailedOrInProgress;
            }

            return SuccessEnrollmentCategory.Passed;
        }

        var lessonsTotal = course.Lessons.Count;
        if (lessonsTotal == 0) return SuccessEnrollmentCategory.Passed;
        var doneLessons = userProgress
            .Where(p => IsStatus(p.Status, "completed"))
            .Select(p => p.LessonId)
            .Distinct()
            .Count();
        return doneLessons >= lessonsTotal
            ? SuccessEnrollmentCategory.Passed
            : SuccessEnrollmentCategory.FailedOrInProgress;
    }

    private static SuccessDistributionDTO BuildSuccessDistribution(AnalyticsScope scope, PeriodicRangeUtc? period)
    {
        IEnumerable<(int CourseId, int UserId, DateTime OrderDateUtc)> orders = scope.EnrollmentOrders;
        if (period != null)
        {
            orders = orders.Where(o =>
                o.OrderDateUtc >= period.FromInclusiveUtc && o.OrderDateUtc < period.ToExclusiveUtc);
        }
        ...
        {
            switch (ClassifyEnrollmentForSuccessPie(scope, e.CourseId, e.UserId, DefaultPassingScorePercent))
            {
                case SuccessEnrollmentCategory.NotStarted:
                    notStarted++;
                    break;
                case SuccessEnrollmentCategory.Passed:
                    passed++;
                    break;
                default:
                    failed++;
        ...
        if (period == null)
            periodDescription = "Все записи (без ограничения по дате заявки).";
        else
        {
            var from = FormatAnalyticsRuDate(period.FromInclusiveUtc);
            var toIncl = FormatAnalyticsRuDate(period.ToExclusiveUtc.AddTicks(-1));
            periodDescription = $"Записи с датой заявки с {from} по {toIncl}.";
        }

        return new SuccessDistributionDTO
        {
            PassingScorePercent = DefaultPassingScorePercent,
            TotalEnrollments = list.Count,
            NotStartedCount = notStarted,
            PassedCount = passed,
            FailedOrInProgressCount = failed,
            PeriodDescription = periodDescription
        };
    }

    private static string FormatAnalyticsRuDate(DateTime utc) =>
        DateTime.SpecifyKind(utc, DateTimeKind.Utc).ToString("dd.MM.yyyy", CultureInfo.GetCultureInfo("ru-RU"));

    private static string SuccessCategoryRu(SuccessEnrollmentCategory c) =>
        c switch
        {
            SuccessEnrollmentCategory.NotStarted => "Не приступил",
            SuccessEnrollmentCategory.Passed => "Сдал (≥ порога)",
            _ => "Не сдал / в процессе"
        };
    ...
    /// <summary>Круговая диаграмма успеваемости по записям на курсы (только администратор).</summary>
    [HttpGet("success-distribution")]
    [ProducesResponseType(typeof(SuccessDistributionDTO), StatusCodes.Status200OK)]
    public async Task<IActionResult> GetSuccessDistribution(
        ...
        [FromQuery] DateTime? toUtc = null)
    {
        ...
        if (scope == null) return Unauthorized();
        if (!scope.IsAdmin)
            return StatusCode(StatusCodes.Status403Forbidden, new { message = "Доступно только администратору." });

        var parseErr = TryParsePeriodicRange(null, fromUtc, toUtc, out var period);
        if (parseErr != null) return parseErr;

        return Ok(BuildSuccessDistribution(scope, period));
    }
